...
`INSTALL_DIR` chooses where the app lands: `/Applications` (all users of the
Mac, the default) or `~/Applications` (the current user only). Both are
first-class locations macOS treats identically for Login Items, the
Accessibility list and LaunchServices, so a per-user install is a real choice,
not a workaround — describe it that way. `install.sh` picks the per-user
location automatically when `/Applications` needs admin rights. **Never suggest
`sudo make install`** — it would build and codesign as root, and the signing
identity lives in the user's login keychain, so it fails outright. `make install
SUDO=sudo` elevates only the copy. The `hooks` snippet is derived from
`INSTALL_DIR`, so it stays correct for any prefix; it used to be hardcoded to
`/Applications` and silently wrong.
...
### 7. The app icon exists for a reason

`LSUIElement` means no Dock tile and no app-switcher entry, so it is tempting
to conclude an icon is pointless. It is not: the app is still listed, with its
icon, under **Login Items** and under **Privacy & Security > Accessibility** —
the latter being a screen every user must visit, because the app cannot
function without that grant. With no `CFBundleIconFile` those lists render a
blank generic placeholder, which reads as broken or untrustworthy at exactly
the moment a user is deciding whether to hand over Accessibility access.

`Resources/AppIcon.icns` is generated by `scripts/make-icon.swift` (`make
icon`) and committed, so a normal build never runs the generator. Each size is
drawn from scratch rather than downsampled, so 16pt and 32pt stay crisp. The
artwork is the HUD's own shape language — three stacked capsules, middle one
lit in tmux's status green.
